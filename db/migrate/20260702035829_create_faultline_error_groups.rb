...
class CreateFaultlineErrorGroups < ActiveRecord::Migration[7.2]
  def change
    ...
    # Wrapped in safety_assured because strong_migrations cannot inspect raw
    # execute() calls. This runs against a table created in this same migration,
    # so it is safe. (Override added for the Rails 7.2 / vendored-faultline setup.)
    if postgresql?
      safety_assured do
        execute <<-SQL.squish
