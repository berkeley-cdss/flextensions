  ...
  module V1
    class RequestsController < BaseController
      ...
      def validate_write_token
        # API::BaseController#require_api_available! already refuses every request
        # outside the test environment (no real API token scheme exists yet), so
        # this only runs in tests. It verifies that *some* Authorization header is
        # present -- a stand-in for the token check the real scheme will perform --
        # so specs can exercise both the missing-token and present-token paths.
